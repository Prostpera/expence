...
const mockQuests: MockQuest[] = [
  ...
  { id: 2, title: 'Quest 2', status: QuestStatus.IN_PROGRESS, createdAt: '2025-10-12', category: 'side_jobs', expReward: 50, goal: 'Goal 2', updatedAt: '2025-10-12' },
  { id: 3, title: 'Quest 3', status: QuestStatus.NEW, createdAt: '2025-10-13', category: 'important', expReward: 75, goal: 'Goal 3', updatedAt: '2025-10-13' },
  ...
  { id: 5, title: 'Quest 5', status: QuestStatus.NEW, createdAt: '2025-10-16', category: 'main_story', expReward: 80, goal: 'Goal 5', updatedAt: '2025-10-16' },
...
function getCompletedQuests(quests: MockQuest[]): MockQuest[] {
  return quests
    .filter((q: MockQuest) => q.status === QuestStatus.COMPLETED)
    .sort((a: MockQuest, b: MockQuest) => {
...
function getRecentlyCreatedQuests(quests: MockQuest[]): MockQuest[] {
  return quests
    .filter((q: MockQuest) => q.status !== QuestStatus.COMPLETED)
    .sort((a: MockQuest, b: MockQuest) => {
...
describe('Recent Quest Activity Logic', () => {
  ...
  it('excludes completed quests from recently created quests and returns only active ones', () => {
    ...
    expect(recentCreated.every((q: MockQuest) => q.status !== QuestStatus.COMPLETED)).toBe(true);
    expect(recentCreated.map((q: MockQuest) => q.title)).toEqual(['Quest 5', 'Quest 3', 'Quest 2']);
  });
});
